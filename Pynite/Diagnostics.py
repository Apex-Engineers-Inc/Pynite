...
def find_mechanism_fixes(model: 'FEModel3D') -> List[Tuple[str, str]]:
    """
    Identify nodes and DOFs that need small rotational springs to fix mechanisms.

    Runs only the mechanism check (not full diagnostics) and returns a list of
    (node_name, dof) tuples where dof is 'RY' or 'RZ' indicating which rotational
    DOF needs a small spring to prevent a mechanism.

    Args:
        model: The FEModel3D to check

    Returns:
        List of (node_name, dof) tuples. Empty if no mechanisms detected.
    """
    fixes: List[Tuple[str, str]] = []

    # Build node release map (same logic as _check_mechanisms)
    node_releases: Dict[str, List[Tuple[str, str]]] = defaultdict(list)
    ...
    for node_name, releases in node_releases.items():
        ...
        if num_members > 0:
            ...
            if my_releases > max_releases:
                fixes.append((node_name, 'RY'))
            if mz_releases > max_releases:
                fixes.append((node_name, 'RZ'))

    return fixes


def apply_mechanism_fixes(model: 'FEModel3D', fixes: List[Tuple[str, str]], stiffness: float = 1.0) -> List[Tuple[str, str]]:
    """
    Apply small rotational springs at mechanism nodes to stabilize the model.

    This mimics RISA's behavior of automatically adding small rotational restraints
    at nodes where moment release mechanisms would otherwise cause instability.

    Args:
        model: The FEModel3D to modify
        fixes: List of (node_name, dof) tuples from find_mechanism_fixes()
        stiffness: Small rotational spring stiffness (moment/radian). Default is 1.0.

    Returns:
        List of (node_name, dof) tuples that were actually applied (skips nodes
        that already have springs on that DOF).
    """
    applied = []
    for node_name, dof in fixes:
        node = model.nodes.get(node_name)
        if node is None:
            continue

        spring_attr = f'spring_{dof}'
        spring = getattr(node, spring_attr)

        # Don't overwrite existing springs
        if spring[0] is not None:
            continue

        # Apply a small rotational spring: [stiffness, direction=None (bidirectional), active=True]
        setattr(node, spring_attr, [stiffness, None, True])
        applied.append((node_name, dof))

    return applied


def remove_mechanism_fixes(model: 'FEModel3D', applied: List[Tuple[str, str]]) -> None:
    """
    Remove previously applied mechanism fix springs.

    Args:
        model: The FEModel3D to clean up
        applied: List of (node_name, dof) tuples that were applied
    """
    for node_name, dof in applied:
        node = model.nodes.get(node_name)
        if node is None:
            continue
        spring_attr = f'spring_{dof}'
        setattr(node, spring_attr, [None, None, None])
